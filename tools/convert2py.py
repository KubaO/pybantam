...
import os
import sys
sys.path.append("../thirdparty/java2python")
...
def runOneOrMany(options):
    ...
    if infile and isinstance(infile, os.PathLike | str) and path.isdir(infile):
        if outfile and isinstance(outfile, os.PathLike | str) and not path.isdir(outfile):
            warning(
        ...
        def walker(dirname, files):
            for name in [name for name in files if name.endswith('.java')]:
                ...
                if outfile and outfile != '-' and isinstance(outfile, os.PathLike | str):
                    full = path.abspath(path.join(outfile, fullname))
                    info('outfile %s', outfile)
...
def runTransform(options):
    ...
    if options.inputfile and isinstance(options.inputfile, os.PathLike | str):
        filein = options.inputfile
    if options.outputfile and isinstance(options.outputfile, os.PathLike | str):
    ...
    module.name = path.splitext(path.basename(filein))[
        0] if filein != '-' else '<stdin>'
    module.walk(tree)
    timed['visit_finish']
    ...
    if options.javaast:
        out = sys.stderr
        # with open("astlog.txt", "a") as out:
        print(filein, file=out)
        tree.dump(out)
        print(file=out)

    if options.pytree:
        out = sys.stderr
        # with open("pylog.txt", "a") as out:
        print(filein, file=out)
        module.dumpRepr(out)
        print(file=out)
...
if __name__ == '__main__':
    inroot = "thirdparty/bantam/src/com/stuffwithstuff/bantam/"
    indir = path.join("..", inroot)
    outdir = "../src/src"
    args = [
        indir, outdir,
        "--log-level", "INFO",
    ]
    ns = configScript(args)
    runOneOrMany(ns)

    for (dirpath, dirnames, filenames) in os.walk(path.join(outdir, "..")):
        for filename in filenames:
            src = path.abspath(path.join(dirpath, filename))
            src = src.replace(os.sep, "/")
            dst = src.replace(inroot, "")
            os.replace(src, dst)
